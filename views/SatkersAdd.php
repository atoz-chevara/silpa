...
loadjs.ready("head", function () {
    ...
    fsatkersadd = currentForm = new ew.Form("fsatkersadd", "add");
    ...
    // Form_CustomValidate
    fsatkersadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsatkersadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fsatkersadd.lists.wilayah = <?= $Page->wilayah->toClientList($Page) ?>;
    fsatkersadd.lists.idd_user = <?= $Page->idd_user->toClientList($Page) ?>;
    loadjs.done("fsatkersadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fsatkersadd" id="fsatkersadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="satkers" data-field="x_idd_user" data-hidden="1" name="x_idd_user" id="x_idd_user" value="<?= HtmlEncode($Page->idd_user->CurrentValue) ?>">
<?php } else { ?>
<span id="el_satkers_idd_user">
    <select
        id="x_idd_user"
