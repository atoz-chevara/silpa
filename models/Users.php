...
class Users extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // idd_user
        $this->idd_user = new DbField('users', 'users', 'x_idd_user', 'idd_user', '`idd_user`', '`idd_user`', 3, 100, -1, false, '`idd_user`', false, false, false, 'FORMATTED TEXT', 'NO');
        $this->idd_user->IsAutoIncrement = true; // Autoincrement field
        $this->idd_user->IsPrimaryKey = true; // Primary key field
        ...
        // username
        $this->_username = new DbField('users', 'users', 'x__username', 'username', '`username`', '`username`', 200, 200, -1, false, '`username`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // password
        $this->_password = new DbField('users', 'users', 'x__password', 'password', '`password`', '`password`', 200, 200, -1, false, '`password`', false, false, false, 'FORMATTED TEXT', 'PASSWORD');
        ...
        switch ($CurrentLanguage) {
            case "en":
                $this->level->Lookup = new Lookup('level', 'levels', false, 'id_level', ["level_name","","",""], [], [], [], [], [], [], '', '');
                break;
            default:
                $this->level->Lookup = new Lookup('level', 'levels', false, 'id_level', ["level_name","","",""], [], [], [], [], [], [], '', '');
                break;
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->idd_user->setDbValue($conn->lastInsertId());
            $rs['idd_user'] = $this->idd_user->DbValue;
        }
        return $success;
    }
    ...
    protected function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('idd_user', $rs)) {
                AddFilter($where, QuotedName('idd_user', $this->Dbid) . '=' . QuotedValue($rs['idd_user'], $this->idd_user->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    public function deleteUploadedFiles($row)
    {
        ...
        $this->photo->OldUploadPath = "files/photo";
    ...
    {
        $keys = [];
        $val = $current ? $this->idd_user->CurrentValue : $this->idd_user->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->idd_user->CurrentValue = $keys[0];
            } else {
                $this->idd_user->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('idd_user', $row) ? $row['idd_user'] : null;
        } else {
            $val = $this->idd_user->OldValue !== null ? $this->idd_user->OldValue : $this->idd_user->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@idd_user@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    {
        $json = "";
        $json .= "idd_user:" . JsonEncode($this->idd_user->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->idd_user->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->idd_user->CurrentValue);
        } else {
...
SORTHTML;
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("idd_user") ?? Route("idd_user")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->idd_user->CurrentValue = $key;
            } else {
                $this->idd_user->OldValue = $key;
            }
    ...
    {
        $userIdFilter = '`idd_user` = ' . QuotedValue($userId, DATATYPE_NUMBER, Config("USER_TABLE_DBID"));
        return $userIdFilter;
    }
    ...
    public function getFileData($fldparm, $key, $resize, $width = 0, $height = 0, $plugins = [])
    {
        ...
        if (count($ar) == 1) {
            $this->idd_user->CurrentValue = $ar[0];
        } else {
