...
class UsersEdit extends Users
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("idd_user") ?? Key(0) ?? Route(2)) !== null) {
                $this->idd_user->setQueryStringValue($keyValue);
                $this->idd_user->setOldValue($this->idd_user->QueryStringValue);
            } elseif (Post("idd_user") !== null) {
                $this->idd_user->setFormValue(Post("idd_user"));
                $this->idd_user->setOldValue($this->idd_user->FormValue);
            } else {
