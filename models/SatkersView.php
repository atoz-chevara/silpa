 ...
 * Page class
 */
class SatkersView extends Satkers
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SatkersView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("idd_satker") ?? Route("idd_satker")) !== null) {
            $this->RecKey["idd_satker"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("idd_satker") ?? Route("idd_satker")) !== null) {
                ...
                $this->RecKey["idd_satker"] = $this->idd_satker->QueryStringValue;
            } elseif (Post("idd_satker") !== null) {
                $this->idd_satker->setFormValue(Post("idd_satker"));
                $this->RecKey["idd_satker"] = $this->idd_satker->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->idd_satker->setQueryStringValue($keyValue);
                $this->RecKey["idd_satker"] = $this->idd_satker->QueryStringValue;
            } else {
                $returnUrl = "satkerslist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "satkerslist"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "satkerslist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
