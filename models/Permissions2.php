...
class Permissions2 extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // table_name
        $this->table_name = new DbField('permissions2', 'permissions', 'x_table_name', 'table_name', '`table_name`', '`table_name`', 200, 100, -1, false, '`table_name`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // id_level
        $this->id_level = new DbField('permissions2', 'permissions', 'x_id_level', 'id_level', '`id_level`', '`id_level`', 3, 11, -1, false, '`id_level`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        switch ($CurrentLanguage) {
            case "en":
                $this->id_level->Lookup = new Lookup('id_level', 'levels', false, 'id_level', ["level_name","","",""], [], [], [], [], [], [], '', '');
                break;
            default:
                $this->id_level->Lookup = new Lookup('id_level', 'levels', false, 'id_level', ["level_name","","",""], [], [], [], [], [], [], '', '');
                break;
        ...
        // permission
        $this->_permission = new DbField('permissions2', 'permissions', 'x__permission', 'permission', '`permission`', '`permission`', 3, 11, -1, false, '`permission`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        switch ($CurrentLanguage) {
            case "en":
                $this->_permission->Lookup = new Lookup('permission', 'levels', false, 'id_level', ["level_name","","",""], [], [], [], [], [], [], '', '');
                break;
            default:
                $this->_permission->Lookup = new Lookup('permission', 'levels', false, 'id_level', ["level_name","","",""], [], [], [], [], [], [], '', '');
                break;
...
SORTHTML;
    ...
    public function renderListRow()
    {
        ...
        $curVal = trim(strval($this->_permission->CurrentValue));
        if ($curVal != "") {
            $this->_permission->ViewValue = $this->_permission->lookupCacheOption($curVal);
            if ($this->_permission->ViewValue === null) { // Lookup from database
                $filterWrk = "`id_level`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
                $sqlWrk = $this->_permission->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->_permission->Lookup->renderViewRow($rswrk[0]);
                    $this->_permission->ViewValue = $this->_permission->displayValue($arwrk);
                } else {
                    $this->_permission->ViewValue = $this->_permission->CurrentValue;
                }
            }
        } else {
            $this->_permission->ViewValue = null;
        }
