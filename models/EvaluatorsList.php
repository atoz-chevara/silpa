 ...
 * Page class
 */
class EvaluatorsList extends Evaluators
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EvaluatorsList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fevaluatorslist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "evaluatorsadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "evaluatorsdelete";
        $this->MultiUpdateUrl = "evaluatorsupdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fevaluatorslistsrch";
    ...
    {
        if (!SameString($this->idd_evaluator->OldValue, $this->idd_evaluator->CurrentValue)) {
            return false;
        }
        return true;
    }
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->idd_evaluator->CurrentValue;
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_nama_lengkap") && $CurrentForm->hasValue("o_nama_lengkap") && $this->nama_lengkap->CurrentValue != $this->nama_lengkap->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_wilayah") && $CurrentForm->hasValue("o_wilayah") && $this->wilayah->CurrentValue != $this->wilayah->OldValue) {
            return false;
        }
    ...
    public function resetFormError()
    {
        ...
        $this->nama_lengkap->clearErrorMessage();
        $this->wilayah->clearErrorMessage();
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->idd_evaluator->AdvancedSearch->toJson(), ","); // Field idd_evaluator
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fevaluatorslistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field idd_evaluator
        $this->idd_evaluator->AdvancedSearch->SearchValue = @$filter["x_idd_evaluator"];
        $this->idd_evaluator->AdvancedSearch->SearchOperator = @$filter["z_idd_evaluator"];
        $this->idd_evaluator->AdvancedSearch->SearchCondition = @$filter["v_idd_evaluator"];
        $this->idd_evaluator->AdvancedSearch->SearchValue2 = @$filter["y_idd_evaluator"];
        $this->idd_evaluator->AdvancedSearch->SearchOperator2 = @$filter["w_idd_evaluator"];
        $this->idd_evaluator->AdvancedSearch->save();
    ...
    protected function loadSortOrder()
    {
        ...
        if ($orderBy == "") {
            $this->DefaultSort = "`wilayah` ASC";
            if ($this->getSqlOrderBy() != "") {
                $useDefaultSort = true;
                if ($this->wilayah->getSort() != "") {
                    $useDefaultSort = false;
                }
                if ($useDefaultSort) {
                    $this->wilayah->setSort("ASC");
    ...
    public function renderListOptions()
    {
        ...
        if ($this->isInlineEditRow()) { // Inline-Edit
            ...
            $opt->Body .= "<input type=\"hidden\" name=\"k" . $this->RowIndex . "_key\" id=\"k" . $this->RowIndex . "_key\" value=\"" . HtmlEncode($this->idd_evaluator->CurrentValue) . "\">";
            return;
        }
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->idd_evaluator->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fevaluatorslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fevaluatorslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit()) { // Not grid add/edit mode
            ...
            foreach ($this->ListActions->Items as $listaction) {
                if ($listaction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fevaluatorslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_nama_lengkap")) {
            $this->nama_lengkap->setOldValue($CurrentForm->getValue("o_nama_lengkap"));
        }
        ...
        if ($CurrentForm->hasValue("o_wilayah")) {
            $this->wilayah->setOldValue($CurrentForm->getValue("o_wilayah"));
        }
        ...
        if (!$this->idd_evaluator->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash .= GetFieldHash($row['nama_lengkap']); // nama_lengkap
        $hash .= GetFieldHash($row['wilayah']); // wilayah
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fevaluatorslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
