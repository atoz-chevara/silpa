 ...
 * Page class
 */
class SatkersList extends Satkers
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SatkersList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fsatkerslist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "satkersadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "satkersdelete";
        $this->MultiUpdateUrl = "satkersupdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fsatkerslistsrch";
    ...
    {
        if (!SameString($this->idd_satker->OldValue, $this->idd_satker->CurrentValue)) {
            return false;
        }
        return true;
    }
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->idd_satker->CurrentValue;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_kode_pemda") && $CurrentForm->hasValue("o_kode_pemda") && $this->kode_pemda->CurrentValue != $this->kode_pemda->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_kode_satker") && $CurrentForm->hasValue("o_kode_satker") && $this->kode_satker->CurrentValue != $this->kode_satker->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_nama_satker") && $CurrentForm->hasValue("o_nama_satker") && $this->nama_satker->CurrentValue != $this->nama_satker->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_idd_user") && $CurrentForm->hasValue("o_idd_user") && $this->idd_user->CurrentValue != $this->idd_user->OldValue) {
            return false;
        }
        return true;
    }
    ...
    {
        $this->kode_pemda->clearErrorMessage();
        $this->kode_satker->clearErrorMessage();
        $this->nama_satker->clearErrorMessage();
        $this->idd_user->clearErrorMessage();
    }
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->idd_satker->AdvancedSearch->toJson(), ","); // Field idd_satker
        $filterList = Concat($filterList, $this->kode_pemda->AdvancedSearch->toJson(), ","); // Field kode_pemda
        ...
        $filterList = Concat($filterList, $this->wilayah->AdvancedSearch->toJson(), ","); // Field wilayah
        $filterList = Concat($filterList, $this->idd_user->AdvancedSearch->toJson(), ","); // Field idd_user
        $filterList = Concat($filterList, $this->no_telepon->AdvancedSearch->toJson(), ","); // Field no_telepon
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fsatkerslistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field idd_satker
        $this->idd_satker->AdvancedSearch->SearchValue = @$filter["x_idd_satker"];
        $this->idd_satker->AdvancedSearch->SearchOperator = @$filter["z_idd_satker"];
        $this->idd_satker->AdvancedSearch->SearchCondition = @$filter["v_idd_satker"];
        $this->idd_satker->AdvancedSearch->SearchValue2 = @$filter["y_idd_satker"];
        $this->idd_satker->AdvancedSearch->SearchOperator2 = @$filter["w_idd_satker"];
        $this->idd_satker->AdvancedSearch->save();

        // Field kode_pemda
        $this->kode_pemda->AdvancedSearch->SearchValue = @$filter["x_kode_pemda"];
        $this->kode_pemda->AdvancedSearch->SearchOperator = @$filter["z_kode_pemda"];
        $this->kode_pemda->AdvancedSearch->SearchCondition = @$filter["v_kode_pemda"];
        $this->kode_pemda->AdvancedSearch->SearchValue2 = @$filter["y_kode_pemda"];
        $this->kode_pemda->AdvancedSearch->SearchOperator2 = @$filter["w_kode_pemda"];
        $this->kode_pemda->AdvancedSearch->save();
        ...
        // Field no_telepon
        $this->no_telepon->AdvancedSearch->SearchValue = @$filter["x_no_telepon"];
        $this->no_telepon->AdvancedSearch->SearchOperator = @$filter["z_no_telepon"];
        $this->no_telepon->AdvancedSearch->SearchCondition = @$filter["v_no_telepon"];
        $this->no_telepon->AdvancedSearch->SearchValue2 = @$filter["y_no_telepon"];
        $this->no_telepon->AdvancedSearch->SearchOperator2 = @$filter["w_no_telepon"];
        $this->no_telepon->AdvancedSearch->save();
    ...
    {
        $where = "";
        $this->buildBasicSearchSql($where, $this->kode_pemda, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->nama_satker, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->no_telepon, $arKeywords, $type);
        return $where;
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->kode_pemda); // kode_pemda
    ...
    public function renderListOptions()
    {
        ...
        if ($this->isInlineEditRow()) { // Inline-Edit
            ...
            $opt->Body .= "<input type=\"hidden\" name=\"k" . $this->RowIndex . "_key\" id=\"k" . $this->RowIndex . "_key\" value=\"" . HtmlEncode($this->idd_satker->CurrentValue) . "\">";
            return;
        }
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->idd_satker->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsatkerslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsatkerslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit()) { // Not grid add/edit mode
            ...
            foreach ($this->ListActions->Items as $listaction) {
                if ($listaction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fsatkerslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_kode_pemda")) {
            $this->kode_pemda->setOldValue($CurrentForm->getValue("o_kode_pemda"));
        }
        ...
        if ($CurrentForm->hasValue("o_kode_satker")) {
            $this->kode_satker->setOldValue($CurrentForm->getValue("o_kode_satker"));
        }
        ...
        if ($CurrentForm->hasValue("o_nama_satker")) {
            $this->nama_satker->setOldValue($CurrentForm->getValue("o_nama_satker"));
        }
        ...
        if ($CurrentForm->hasValue("o_idd_user")) {
            $this->idd_user->setOldValue($CurrentForm->getValue("o_idd_user"));
        }
        ...
        if (!$this->idd_satker->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['kode_pemda']); // kode_pemda
        $hash .= GetFieldHash($row['kode_satker']); // kode_satker
        $hash .= GetFieldHash($row['nama_satker']); // nama_satker
        $hash .= GetFieldHash($row['idd_user']); // idd_user
        return md5($hash);
    }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fsatkerslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
